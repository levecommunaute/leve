  ...
  const loadProfil = useCallback(async (activeSession: Session) => {
    ...
    if (
      ...
      const retry = await supabase
        ...
        .limit(5);
      if (!retry.error) {
        quizSubs = (retry.data ?? []) as QuizSubmissionRow[];
        quizErr = null;
      }
    }
    ...
    if (quizErr) {
      ...
      if (ids.length > 0) {
        const vRes = await supabase.from("videos").select("id, title").in("id", ids);
        if (!vRes.error && vRes.data) {
          titles = new Map(
            vRes.data.map((v) => [String(v.id), String(v.title ?? "")]),
          );
        }
      }
      setQuizRows(
              ...
              {quizRows.map((row, i) => (
                ...
                >
                  <div>
                    ...
                    {row.at ? (
                      <p style={{ margin: "0.35rem 0 0", fontSize: "0.8rem", opacity: 0.55 }}>
          ...
          {navPages.map((p) => (
            <Link
              ...
              style={{
                flex: "0 0 auto",
                fontSize: "0.68rem",
                color: p.href === "/profil" ? GOLD : TEXT,
                opacity: p.href === "/profil" ? 1 : 0.75,
                textDecoration: "none",
